...
const MAX_MOUNTED_TRAJECTORY_ROWS = 160
...
interface TrajectoryRowCounts {
  readonly ariaRows: number
  readonly logicalRows: number
  readonly historyLoadRows: number
  readonly mountedRows: number
}

async function stableTrajectoryRows(
  page: Page,
  accepts: (counts: TrajectoryRowCounts) => boolean,
  timeoutMs = 60_000,
): Promise<TrajectoryRowCounts> {
  ...
  const mounted = page.locator('[data-trajectory-scroll] tr[data-trajectory-row-key]')
  const historyLoad = page.locator('[data-trajectory-scroll] tr[data-history-load]')
  const deadline = performance.now() + timeoutMs
  let previous: TrajectoryRowCounts = {
    ariaRows: -1,
    logicalRows: -1,
    historyLoadRows: -1,
    mountedRows: -1,
  ...
  while (performance.now() < deadline) {
    ...
    const ariaRows = Number(raw)
    const historyLoadRows = await historyLoad.count()
    const current = {
      ariaRows,
      logicalRows: ariaRows - historyLoadRows,
      historyLoadRows,
      mountedRows: await mounted.count(),
    }
    stableReads = accepts(current)
      && current.ariaRows === previous.ariaRows
      && current.logicalRows === previous.logicalRows
      && current.historyLoadRows === previous.historyLoadRows
      && current.mountedRows === previous.mountedRows
      ? stableReads + 1
      : 0
  ...
  throw new Error(`trajectory row counts did not stabilize; last counts ${JSON.stringify(previous)}`)
}
...
async function openPerformancePage(world: PerformanceWorld): Promise<Locator> {
  ...
  const tree = world.page.getByRole('tree', { name: 'Sessions' })
  const group = tree.getByRole('treeitem').first()
  // Compact Workspace rows intentionally omit Session counts. The Ungrouped
  // row appears only after the atomic Session-list response has been reduced.
  await expect.poll(() => group.textContent(), { timeout: 30_000 }).toContain('Ungrouped')
  await expect.poll(() => group.getAttribute('aria-expanded'), { timeout: 30_000 }).toBe('false')
  return group
}
  ...
  it('reports workspace, history, and trajectory cardinality costs', async () => {
    ...
    try {
      ...
      const coldTrajectory = await measure(cdp, async () => {
        ...
        return stableTrajectoryRows(page, counts => counts.logicalRows > 0
          && counts.logicalRows < EXPECTED_TRAJECTORY_ROWS
          && counts.mountedRows > 0
          && counts.mountedRows <= MAX_MOUNTED_TRAJECTORY_ROWS
          && counts.mountedRows < counts.logicalRows)
      })

      const collapseTurns = await measure(cdp, async () => {
        await page.getByRole('button', { name: 'Collapse turns', exact: true }).click()
        return stableTrajectoryRows(page, counts => counts.logicalRows > 0
          && counts.logicalRows < coldTrajectory.value.logicalRows
          && counts.mountedRows <= MAX_MOUNTED_TRAJECTORY_ROWS)
      })
      const trajectorySearch = await measure(cdp, async () => {
        ...
        return stableTrajectoryRows(page, counts => counts.logicalRows > 0
          && counts.logicalRows < collapseTurns.value.logicalRows
          && counts.mountedRows <= counts.logicalRows)
      })
      await page.getByRole('searchbox', { name: 'Search trajectory', exact: true }).fill('')
      const expandTurns = page.getByRole('button', { name: 'Expand turns', exact: true })
      const collapseTurnsButton = page.getByRole('button', { name: 'Collapse turns', exact: true })
      await expect.poll(async () => await expandTurns.count() + await collapseTurnsButton.count())
        .toBe(1)
      if (await expandTurns.count() === 1) await expandTurns.click()
      await stableTrajectoryRows(page, counts => counts.logicalRows >= coldTrajectory.value.logicalRows
        && counts.logicalRows <= EXPECTED_TRAJECTORY_ROWS
        && counts.mountedRows > 0
        && counts.mountedRows <= MAX_MOUNTED_TRAJECTORY_ROWS
        && counts.mountedRows < counts.logicalRows)
      ...
      const warmTrajectory = await measure(cdp, async () => {
        await page.getByRole('tab', { name: 'Trajectory', exact: true }).click()
        return stableTrajectoryRows(page, counts => counts.logicalRows === EXPECTED_TRAJECTORY_ROWS
          && counts.mountedRows > 0
          && counts.mountedRows <= MAX_MOUNTED_TRAJECTORY_ROWS
          && counts.mountedRows < counts.logicalRows)
      })
