// Anime profile visual acceptance matrix. The committed test is the matrix;
...
const ANIME_OVERLAY = join(REPO_ROOT, 'packages', 'bundle', 'anime-desktop', 'cordis.patch.yml')
const ANIME_INSTALL_ANCHOR = join(REPO_ROOT, 'apps', 'desktop', 'package.json')
const ARTIFACT_ROOT = join(REPO_ROOT, 'artifacts', 'qa', 'anime-visual-matrix')
...
async function mutateSettings(
  ...
  await scaffold.ctx.settings.mutate(settingsNamespace('ui-anime'), [
...
async function captureVisual(options: CaptureOptions): Promise<void> {
  ...
  const geometry = await options.page.evaluate(() => {
    ...
    const conversation = document.querySelector<HTMLElement>('[data-anime-conversation]')
    const sidebar = document.querySelector<HTMLElement>('[data-anime-sidebar]')
    if (frameElement === null || conversation === null || sidebar === null) {
      throw new Error('anime visual matrix: frame seats are missing')
    }
...
async function closeWorld(browser: Browser | undefined, scaffold: WebScaffold | undefined): Promise<void> {
  ...
  if (failures.length > 1) throw new AggregateError(failures, 'anime visual matrix teardown failed')
}
...
const TOOL_SCRIPT: ReplayEntry[] = [
  {
    ...
    chunks: [
      { type: 'block-start', index: 0, blockType: 'tool-call' },
      {
        ...
        block: {
          type: 'tool-call',
          id: CallId('call_anime_visual_tool'),
          name: process.platform === 'win32' ? 'pwsh' : 'bash',
          arguments: JSON.stringify({
            command: process.platform === 'win32'
              ? "Start-Sleep -Seconds 4; Write-Output 'ANIME_VISUAL_TOOL_OK'"
              : 'node -e "setTimeout(function(){console.log(\'ANIME_VISUAL_TOOL_OK\')},4000)"',
...
describe.skipIf(MODE === 'record')('anime profile visual acceptance matrix', () => {
  ...
  it('covers every viewport, Windows scale, theme, mode, motion, visibility, sidebar, and locale value', async () => {
    const scaffold = await launchWebScaffold({
      ...
      extraOverlayPath: ANIME_OVERLAY,
      installAnchor: ANIME_INSTALL_ANCHOR,
    })
  ...
  it('captures ordinary and long history, completed tools, details, hidden character, and a stable collapsed sidebar', async () => {
    ...
    await seedSession(scaffold, realizeSeedFixture(scaffold, raw, 'anime-visual-history'), 'anime-visual-history')
    await mutateSettings(scaffold, {
    ...
    try {
      ...
      await page.locator('[data-anime-details][data-open]').waitFor({ timeout: 10_000 })
      await captureVisual({
      ...
      await page.locator('[data-anime-details]').getByRole('button', { name: 'Close details' }).click()
      ...
      await expect.poll(() => page.locator('[data-character-state]').getAttribute('data-character-visible'))
        .toBeNull()
      const collapse = page.locator('[data-anime-sidebar]').getByRole('button', { name: 'Collapse sidebar' })
      await collapse.click()
      await expect.poll(() => page.locator('[data-anime-sidebar]').getAttribute('data-collapsed')).toBe('true')
      await captureVisual({
  ...
  it('captures a real running tool and its success transition', async () => {
    ...
    if (prompt === undefined) throw new Error('anime visual tool fixture has no user prompt')
    const scaffold = await launchWebScaffold({
      nativeWindowsShell: true,
      extraOverlayPath: ANIME_OVERLAY,
      installAnchor: ANIME_INSTALL_ANCHOR,
  ...
  it('captures approval, question, and terminal error states through real replayed product flows', async () => {
    ...
    if (errorPrompt === undefined) throw new Error('anime visual error fixture has no user prompt')
