...
describe('profile boot install anchor', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.resolveProfileDir.mockReturnValue('E:\\profiles\\lumi-desktop')
    mocks.loadProfile.mockReturnValue({
      dir: 'E:\\profiles\\lumi-desktop',
      patchPath: 'E:\\profiles\\lumi-desktop\\cordis.patch.yml',
      patches: [],
      layers: [],
    })
  })
  ...
  it('initializes a missing embedded profile from the requested bundle stack', () => {
    const bundles = ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app', '@dsh-lumi/bundle-desktop']
    prepareProfile('lumi-desktop', true, 'E:\\apps\\desktop\\package.json', bundles)
    expect(mocks.resolveProfileDir).toHaveBeenCalledWith('lumi-desktop')
    expect(mocks.initProfile).toHaveBeenCalledWith('E:\\profiles\\lumi-desktop', bundles)
  })

  it('heals and loads bundles from an embedded launcher manifest', () => {
    const anchor = 'E:\\apps\\desktop\\package.json'
    const profile = prepareProfile('lumi-desktop', false, anchor)

    expect(mocks.healProfilesModuleFallback).toHaveBeenCalledWith(anchor)
    expect(mocks.loadProfile).toHaveBeenCalledWith(
      'dsh', 'lumi-desktop', anchor, undefined, { userLayer: false },
    )
    expect(mocks.writeFileSync).toHaveBeenCalledWith(
      `E:\\profiles\\lumi-desktop\\${PROFILE_ROOT_FILENAME}`,
