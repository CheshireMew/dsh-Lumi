...
The Chat and Trajectory Assistant Definitions still fold every raw chunk into their independently owned state machines, but visibility discovery is incremental. Until the first visible content appears, a text or reasoning delta checks only its incoming text, while block start and block end inspect only the changed block. Once `firstVisibleSeq` is set, later deltas preserve that fact without trimming the accumulated block. Complete-content scans remain at animation-frame projection and finalization, where they run at publication frequency rather than once per raw event. This preserves whitespace-only, tool-only, retry, interruption, and replay behavior while preventing stream length from multiplying ingestion work.
...
Focused tests pin `Notifier`'s per-frame coalescing, structural-event preemption, invalidated callbacks, and no-rAF fallback, and prove at the `Session` layer that a frame publishes the latest accumulated text only once and that finalization is not followed by a duplicate notification from a stale frame callback. Both Assistant Definitions also prove that later visible deltas do not call `String.prototype.trim`, so the default suite guards the ingestion cost directly without timing assumptions. Small fixture unit tests continue to pin input validation, external arrival pacing, concurrency rejection, exact event count, and terminal-marker delivery without bringing the 100,000-chunk workload into the default test suites.
...
**Replacing streamed block text with a segmented buffer.** Rejected for the current workload: removing per-event full-content scans brings the unchanged 100,000-chunk browser workload within both responsiveness budgets while ordinary string append preserves the public `AssistantBlock` representation. A second private representation would add materialization and lifecycle rules without deleting any remaining measured bottleneck.
...
The publication rate of streaming `ConversationSnapshot` objects is bounded by the browser's paint rate, so React handles at most one accumulated partial containing all received text per frame; structural events can still publish sooner. Ingestion, ordering, logging, string concatenation, and accumulator updates still run for every raw chunk, but accumulated-text scans do not. The browser stress lane therefore covers both per-frame publication and per-event reduction cost without discarding or combining session facts.
