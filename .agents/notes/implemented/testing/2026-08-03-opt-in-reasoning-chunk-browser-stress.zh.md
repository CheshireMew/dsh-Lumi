...
Chat 和 Trajectory 的 Assistant Definition 仍会把每个原始分片折叠进各自独立拥有的状态机，但可见内容采用增量识别。在第一段可见内容出现前，文本或推理增量只检查本次传入的文本，块开始和块结束只检查发生变化的块。设置 `firstVisibleSeq` 后，后续增量直接保留这个事实，不再对累计块执行 `trim`。完整内容扫描只留在动画帧投影与定稿阶段，执行频率由发布频率决定，而不是按每个原始事件执行。空白、纯工具调用、重试、中断和重放语义都保持不变，同时流长度不再放大会话接收工作量。
...
聚焦测试固定 `Notifier` 的逐帧合并、结构事件抢占、失效回调和无 rAF 回退，并在 `Session` 层证明一帧只发布一次最新累计文本且定稿不会被旧帧回调重复通知。两套 Assistant Definition 还会证明首段可见内容之后的增量不调用 `String.prototype.trim`，默认测试套件因而能直接守住接收成本，无需依赖耗时断言。fixture（测试前置数据）的小型单元测试继续固定输入校验、外部到达节奏、并发拒绝、精确事件数和结尾标记交付，无需把 100,000 分片工作负载带入默认测试套件。
...
**把流式块文本替换为分段缓冲区。** 当前工作负载下不予采纳：移除逐事件全文扫描后，保持 100,000 分片不变的浏览器工作负载已经同时满足两项响应预算，普通字符串追加也能保留公开的 `AssistantBlock` 表示。引入第二套私有表示会增加物化与生命周期规则，却不能继续删除已测得的瓶颈。
...
流式 `ConversationSnapshot` 的发布频率受浏览器绘制频率约束，React 每帧至多处理一个包含全部已接收文本的累计 partial；结构事件仍可更快发布。接收、排序、日志记录、字符串拼接和累积器更新仍按原始分片执行，但累计全文扫描不会。浏览器压力车道因此会同时覆盖逐帧发布和逐事件归并成本，且不会丢弃或合并会话事实。
