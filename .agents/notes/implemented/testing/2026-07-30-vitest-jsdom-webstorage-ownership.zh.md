...
仓库改为只支持 Node 22 之前，声明的版本范围包含会预留进程级 `globalThis.localStorage` 的 Node 26。未设置 `--localstorage-file` 时，Node 26 将该属性暴露为 `undefined`；Vitest 检测到这个预留键后，不会用 jsdom 的隔离 `Storage` 对象覆盖该属性。因此，组件测试套件尚未验证产品行为便会失败，而原来的 Node 24 覆盖率通道仍能通过。当前 Node 22 版本线不会触发该故障，但这项条件隔离仍作为未来扩展运行时前的回归保护。
...
Node 下限兼容性汇总任务会在每条声明支持的版本线上运行专用的 jsdom 冒烟测试；当前只有 Node 22 下限。该测试同时断言 worker 参数按条件传入且存储可用，因此未来 Node 或 Vitest 变化时，不能在未验证存储行为的情况下扩展运行时支持。
