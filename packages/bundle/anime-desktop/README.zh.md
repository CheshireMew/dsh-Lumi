...
这是叠加在官方 `@deepseek-ai/dsh-base` 和 `@deepseek-ai/dsh-web-app` 组合包之后的产品层。它的 patch 只插入 `@dsh-anime/client-character` 与 `@dsh-anime/client-shell`，因此同步上游时可以完整审查产品配置项的差异。
