...
import type { LumiAnimationDefinition, LumiCharacterStateDefinition, LumiPackManifest } from './pack-contract.ts'
import { BUILTIN_PACK_ID } from './settings.ts'

const STILL: LumiAnimationDefinition = Object.freeze({
 ...
 */
export function createBuiltinManifest(): LumiPackManifest {
  const state = (expression: string, minDurationMs = 2400): LumiCharacterStateDefinition => ({
    expression,
    fallback: 'idle',
