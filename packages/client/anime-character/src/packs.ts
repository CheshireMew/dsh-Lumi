...
import {
  CHARACTER_STATES, type AnimeAnimationDefinition, type AnimeAnimationKeyframe,
  type AnimeBondUnlock, type AnimeCharacterStateDefinition, type AnimePackCatalogView,
  type AnimePackManifest, type AnimePackView, type CharacterState,
...
interface CatalogSnapshot extends AnimePackCatalogView {
...
function animation(value: unknown): AnimeAnimationDefinition | undefined {
  ...
  const keyframes: AnimeAnimationKeyframe[] = []
  let previous = -1
  for (const candidate of value['keyframes']) {
    ...
    const frame: AnimeAnimationKeyframe = { at: candidate['at'] }
  ...
  return {
    keyframes,
    loop: value['loop'] as AnimeAnimationDefinition['loop'],
...
function assets(root: string, value: unknown): { manifest: AnimePackManifest['assets']; resolved: Map<string, string> } | undefined {
  if (!record(value)
...
function states(
  value: unknown,
  packAssets: AnimePackManifest['assets'],
): Readonly<Record<CharacterState, AnimeCharacterStateDefinition>> | undefined {
  if (!record(value)) return undefined
  const parsed = {} as Record<CharacterState, AnimeCharacterStateDefinition>
...
function idleActions(
  value: unknown,
  packAssets: AnimePackManifest['assets'],
): AnimePackManifest['idleActions'] | undefined {
  if (!record(value)) return undefined
  const parsed: Record<string, { effect?: string; animation: AnimeAnimationDefinition }> = {}
...
function bondUnlocks(
  value: unknown,
  packAssets: AnimePackManifest['assets'],
  actions: AnimePackManifest['idleActions'],
): readonly AnimeBondUnlock[] | undefined {
  if (!Array.isArray(value)) return undefined
  const parsed: AnimeBondUnlock[] = []
  let lastLevel = 0
...
export class AnimePackCatalog {
  private revision = 0
  private watcher: FSWatcher | undefined
  private readonly eventClients = new Set<ServerResponse>()

  /** @param root - `$DSH_HOME/anime/packs`. @param builtinAssetsRoot - shipped original placeholder layers. */
  constructor(
  ...
  snapshot(): CatalogSnapshot {
    ...
    const packs: AnimePackView[] = [packView(builtin, '/anime/assets/builtin-lumi', builtinResolved, assets)]
    const diagnostics: AnimePackCatalogView['diagnostics'][number][] = []
    ...
    for (const entry of entries) {
      ...
      if (loaded === undefined) {
        ...
        this.logger.warn(`anime-character: ignored invalid pack at ${entry.name}: ${message}`)
        continue
      }
      if (ids.has(loaded.manifest.id)) {
        const message = `duplicate pack id ${loaded.manifest.id}`
        diagnostics.push({ directory: entry.name, message })
        this.logger.warn(`anime-character: ignored pack at ${entry.name}: ${message}`)
        continue
      }
      ids.add(loaded.manifest.id)
      packs.push(packView(loaded.manifest, `/anime/assets/packs/${segment(loaded.manifest.id)}`, loaded.resolved, assets))
    }
  ...
  register(server: WebServer): () => void {
    const disposers = [
      server.register({
        kind: 'exact', path: '/anime/packs', handler: (req, res) => {
      ...
      server.register({
        kind: 'exact', path: '/anime/packs/events', handler: (req, res) => {
      ...
      server.register({
        kind: 'prefix', path: '/anime/assets', handler: (req, res) => {
          if (req.method !== 'GET' && req.method !== 'HEAD') { res.writeHead(405); res.end(); return }
