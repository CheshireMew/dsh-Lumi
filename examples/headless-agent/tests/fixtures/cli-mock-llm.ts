...
/** Keyless headless-agent adapter: one real platform shell call followed by a final answer. */
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (toolResult === undefined) {
      ...
      const command = shell === 'pwsh'
        ? '[Console]::Out.Write("CLI_TOOL_ROUND_TRIP")'
        : 'printf CLI_TOOL_ROUND_TRIP'
      const args = JSON.stringify({ command, description: 'Prove the CLI tool round trip.' })
      yield { type: 'block-start', index: 0, blockType: 'tool-call' }
      yield { type: 'tool-call-delta', index: 0, id: CallId('cli-smoke-call'), name: shell, argumentsDelta: args }
      yield { type: 'block-end', index: 0, block: { type: 'tool-call', id: CallId('cli-smoke-call'), name: shell, arguments: args } }
