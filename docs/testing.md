...
- **Unit** (`pnpm run test`): vitest runs package and example `tests/**` plus `scripts/**/*.spec.ts`. pnpm pins development scripts to Node 22.23.2 while published packages keep the wider `engines.node` range, avoiding a Node 24 CJS-lexer worker crash. Windows runs eight sequential one-worker shards; other platforms and explicit filters use one invocation. Every registry gets an HMR cleanup test. Prefer edge cases, error paths, event order, races, and permanent contract regressions (see `packages/core/agent-loop/tests/contract-regressions.spec.ts`).
