...
- **Unit** (`pnpm run test`): vitest over package and example specs under their `tests/**` directories plus repository script specs under `scripts/**/*.spec.ts`; tests stay with the code area they exercise. pnpm installs and locks Node 22.23.2 through the root `devEngines.runtime` declaration for development scripts, while published packages retain the wider `engines.node` range. This avoids a Node 24 CJS-lexer native crash that can terminate isolated Vitest workers. Windows runs the same inventory as eight sequential Vitest shards with one fork worker each, bounding process lifetime and process creation around real-CLI, Git, compiler, and subprocess suites; other platforms use one invocation, and an explicit test filter stays one invocation everywhere. Every registry gets an HMR-safety test (dispose the contributing fiber, assert cleanup). Prefer edge cases, error paths, event ordering, concurrency races, and permanent tests for contract regressions (see `packages/core/agent-loop/tests/contract-regressions.spec.ts`).
