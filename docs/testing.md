...
- **Unit** (`pnpm run test`): Node 22.23.2 avoids Node 24's CJS-lexer worker crash. Windows runs eight sequential shards; fork-isolated suites use two workers and process-bound suites use one. Explicit filters remain one invocation.
- **Coverage gate** (`pnpm run test:coverage`): the milestone and CI run, with per-file 100% on `packages/*/*/src`. Its two-lane runner instruments source evidence while running heavy suites with no unique threshold contribution uninstrumented. Coverage proves execution, not product completion. Hosts without `pwsh` exempt its executor source; CI enforces it.
...
Before local tests, inspect the latest completed failed public CI run once. Active development validates one logical batch on the current host with focused consumers. Full coverage, E2E, matrices, packaging, signing, release, and other platforms require a freeze, release candidate, systemic failure, or explicit request; checks over 15 minutes need approval. Push success ends the task without polling Actions ([decision](../.agents/notes/implemented/process/2026-08-16-development-stage-verification.md)).
