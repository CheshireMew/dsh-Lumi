...
| `tsconfig.host.json` | Host aggregate: Host packages, examples, tests, scripts, website, and the Host leaves of split packages. | Yes |
| `tsconfig.client.json` | Client aggregate: `packages/client/*` packages and their tests, `apps/web`, and the Client leaves of split packages. | Yes |
...
- A new package is registered in exactly one aggregate unless its source owns incompatible Host and Client programs. Having both a Node loader entry and a browser entry is not enough; an ordinary Client plugin produces both runtime artifacts during the Client build phase.

Two packages have split Host and Client tsconfigs. `api/remotes` splits because its Host entry participates in the Host Typert graph while its Client entry imports `/remote` declarations that Host tsdown must generate first. `lumi-character` splits because its Host files own local settings, filesystem-backed packs, and Web routes while its Client files own DOM speech and UI state; bond, manifest, and settings definitions are deliberately listed in both leaves. Each package-root `tsconfig.json` is only a solution, and aggregates and direct consumers reference `tsconfig.host.json` or `tsconfig.client.json`. The workspace `constraints` check uses each reachable project's declared compiler face: shared single-config projects can participate in either aggregate, a single-config project extending `tsconfig.base.client.json` cannot enter the Host graph, and a split target must name the leaf matching its referencing project rather than the solution root or opposite leaf. It discovers split packages from the presence of both leaf configs, so a new split joins the check automatically. The [`api-remotes` README](../packages/api/remotes/README.md) explains the generated-contract ordering details.
