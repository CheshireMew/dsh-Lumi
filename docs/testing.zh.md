...
- **单元测试**（`pnpm run test`）：vitest 运行包和示例的 `tests/**` 以及 `scripts/**/*.spec.ts`。pnpm 把开发脚本锁定到 Node 22.23.2，发布包仍保留更宽的 `engines.node` 范围，以避开 Node 24 CJS lexer 的 worker 原生崩溃。Windows 使用 8 个依次执行的单 worker shard；其他平台和显式过滤只执行一次。每个注册表都有 HMR（热模块替换）清理测试。优先覆盖边界情况、错误路径、事件顺序、竞态和永久约定回归（见 `packages/core/agent-loop/tests/contract-regressions.spec.ts`）。
