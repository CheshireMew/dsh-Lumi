...
- **单元测试**（`pnpm run test`）：Node 22.23.2 避开 Node 24 的 CJS lexer worker 崩溃。Windows 依次运行 8 个 shard；fork 隔离套件使用两个 worker，进程敏感套件使用一个。显式过滤仍只执行一次。
- **覆盖率门禁**（`pnpm run test:coverage`）：里程碑和 CI 对 `packages/*/*/src` 执行逐文件 100% 门禁。双通道执行器为源码证据插桩，不提供独有阈值贡献的重型套件则不插桩。覆盖率只证明执行，不证明产品完成。缺少 `pwsh` 的主机会豁免其执行器源码，CI 仍执行门禁。
...
本地测试前只读取一次公开 CI 最近完成的失败。活跃开发在当前主机用聚焦消费者验证一个逻辑批次。全量覆盖率、E2E、矩阵、打包、签名、发布和其它平台只在冻结、发布候选、系统性失败或用户明确要求时验证；超过 15 分钟先确认。推送成功后立即结束，不轮询 Actions（见[决定](../.agents/notes/implemented/process/2026-08-16-development-stage-verification.md)）。
