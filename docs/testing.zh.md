...
- **单元测试**（`pnpm run test`）：vitest 运行包和示例的 `tests/**` 以及 `scripts/**/*.spec.ts`。开发环境和 CI 统一使用 Node 22.23.2，`engines.node` 也只保留 Node 22 版本线，以避开 Node 24 CJS lexer 的 worker 原生崩溃。Windows 使用 8 个依次执行的单 worker shard；其他平台和显式过滤只执行一次。每个注册表都有 HMR（热模块替换）清理测试。优先覆盖边界情况、错误路径、事件顺序、竞态和永久约定回归（见 `packages/core/agent-loop/tests/contract-regressions.spec.ts`）。
