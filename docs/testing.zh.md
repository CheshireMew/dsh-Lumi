...
- **单元测试**（`pnpm run test`）：vitest 运行包和示例各自的 `tests/**` 目录下的测试，以及匹配 `scripts/**/*.spec.ts` 的仓库脚本测试；测试文件与其所覆盖的代码区域放在一起。根 `devEngines.runtime` 声明让 pnpm 为开发脚本安装并锁定 Node 22.23.2，发布包仍保留更宽的 `engines.node` 范围；这样可以避开 Node 24 CJS lexer 原生崩溃导致隔离 Vitest worker 被终止的问题。Windows 会把同一份测试清单分成 8 个依次执行的 Vitest shard，每个 shard 使用 1 个 fork worker，从而限制真实 CLI、Git、编译器与子进程测试所在进程的寿命和进程创建压力；其他平台仍执行一次，显式测试过滤在所有平台上也只执行一次。每个注册表都有一个 HMR（热模块替换）安全测试（对向该注册表贡献内容的 fiber 执行 dispose（资源释放），并断言清理完成）。优先覆盖边界情况、错误路径、事件顺序、并发竞态，以及针对约定回归的永久测试（见 `packages/core/agent-loop/tests/contract-regressions.spec.ts`）。
