...
const built = spawnSync(pnpm, ['run', 'build:lumi'], { cwd: root, env: environment, stdio: 'inherit', shell: false })
if (built.status !== 0) process.exit(built.status ?? 1)

const children: ChildProcess[] = [
  start(pnpm, ['--filter', '@deepseek-ai/dsh-web-frontend', 'run', 'watch']),
  start(pnpm, ['--filter', '@dsh-lumi/client-character', 'run', 'watch']),
  start(pnpm, ['--filter', '@dsh-lumi/client-shell', 'run', 'watch']),
  start(pnpm, ['--filter', '@dsh-lumi/desktop', 'exec', 'tsc', '-b', '--watch', '--preserveWatchOutput']),
  start(pnpm, ['--filter', '@dsh-lumi/desktop', 'exec', 'tsdown', '--watch']),
]
